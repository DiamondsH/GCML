...
class LSLRGradientDescentLearningRule(nn.Module):
    def __init__(self, device, total_num_inner_loop_steps, init_learning_rate=1e-5, use_learnable_lr=True, lr_of_lr=1e-3,use_learnable_drop_p=True, drop_p=0.8):
        ...
        self.drop_p = torch.tensor(drop_p, requires_grad=True)
    ...
    def update_lrs_modify(self, loss, scaler=None,args=None):
        if self.use_learnable_lr:
            if scaler is not None:
                scaled_grads = torch.autograd.grad(scaler.scale(loss), self.names_learning_rates_dict.values())
                inv_scale = 1. / scaler.get_scale()
